...
@section('subheader', 'Submit a new time off request')
    ...
    <form action="{{ route('employee.leave-requests.store') }}" method="POST" class="space-y-6">
        ...
        <div>
            ...
            <select id="leave_type" name="leave_type" class="w-full px-4 py-3 rounded-lg border border-gray-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 transition bg-white cursor-pointer @error('leave_type') border-red-500 @enderror">
                ...
                <option value="sick" @selected(old('leave_type') === 'sick')>🏥 Sick Leave</option>
                <option value="vacation" @selected(old('leave_type') === 'vacation')>🏖️ Vacation</option>
                <option value="personal" @selected(old('leave_type') === 'personal')>👤 Personal Leave</option>
                <option value="emergency" @selected(old('leave_type') === 'emergency')>🚨 Emergency Leave</option>
        ...
        <div class="space-y-3">
            ...
            <div class="grid grid-cols-2 gap-3">
                ...
                <div>
                    ...
                    <input type="date" id="start_date" name="start_date" value="{{ old('start_date') }}" class="w-full px-3 py-2.5 rounded-lg border border-gray-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 transition text-sm @error('start_date') border-red-500 @enderror">
                    @error('start_date')
                ...
                <div>
                    ...
                    <input type="date" id="end_date" name="end_date" value="{{ old('end_date') }}" class="w-full px-3 py-2.5 rounded-lg border border-gray-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 transition text-sm @error('end_date') border-red-500 @enderror">
                    @error('end_date')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                </div>
        ...
        <!-- Days Count -->
        <div class="text-sm">
            <p class="text-gray-600">Total Duration: <span id="days-count" class="font-bold text-blue-600">0</span> days</p>
        </div>
        ...
        <div>
            ...
            <textarea id="reason" name="reason" rows="4" placeholder="Explain briefly why you need this leave..." class="w-full px-4 py-3 rounded-lg border border-gray-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 transition resize-none text-sm @error('reason') border-red-500 @enderror">{{ old('reason') }}</textarea>
        ...
        <div class="flex gap-3 pt-4 border-t border-gray-200">
            ...
            <a href="{{ route('employee.leave-requests.index') }}" class="flex-1 px-4 py-3 rounded-lg bg-gray-100 hover:bg-gray-200 text-gray-700 font-semibold transition text-center">
                Cancel
            </a>
        </div>
    </form>
